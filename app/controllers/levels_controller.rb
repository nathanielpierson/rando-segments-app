...
def index
  ...
  def update
    ...
    while x < @level.runs.length
      ...
      minutes_array.push(run_id: @level.runs[x].id, run_minutes: @level.runs[x].minutes, run_seconds: @level.runs[x].seconds, run_milliseconds: @level.runs[x].milliseconds)
      x += 1
    end
    ...
    array_sorted.length.times do |n|
      ...
      p avg_time_in_secs
      avg_time_in_secs += (array_sorted[n][:run_seconds])
    ...
    avg_time_min_calc = (avg_time_in_secs / 60).floor
    avg_time_sec_calc = (avg_time_in_secs % 60).round(2)
    @level.update(
      name: @level.name,
      category: @level.category,
      fastest_time_id: array_sorted[0][:id],
      ...
      avg_time_min: avg_time_min_calc,
      avg_time_sec: avg_time_sec_calc
    )
    render :show
  end
  def fastest_time
